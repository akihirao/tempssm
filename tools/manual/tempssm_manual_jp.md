...
次数を 1 から 3 まで変化させた 3
つのモデルを当てはめます。１次のモデルは
...
結果の要約は、演習 I でおこなった
`summary(res_ar2)`と同じものとなるはずです。 事前にAR(1) から AR(3)
までの自己回帰次数を検討した結果、ベースラインモデルのみ ならず、外生
PDO 指数を含むモデルのいずれにおいても、AR(2) が最良のモデル適合を
...
指数の推定係数は負（-0.44）であり、 その 95% 信頼区間はゼロを含みません
``` math
-0.53, -0.35
```
。これは、山口県沖の 局所的な SST と PDO
の変動との間に、統計的に有意な関係があることを示しています。

具体的な解釈として、基礎となる長期トレンド、季節周期、自己回帰的依存性を
考慮した後でも、PDO 指数が 1 単位増加すると、月別 SST は平均して約 0.44
°C 低下することがモデルから示唆されます。この結果は、PDO
の正の位相が、調査地点における低温の条件に体系的に寄与することを意味します。
...
次に、赤池情報量規準（AIC）を用いて、2
つのモデルの全体的な適合度を比較します。

### AIC に基づくモデル比較

AIC
のようなモデル選択規準は、適合度とモデル複雑性のバランスを考慮した、モデルの
...
示しています。この結果は、PDO
の変動を明示的に考慮することで、内部動態のみで
説明する場合よりも、温度時系列の統計的記述が改善されるという結論を支持します。

AIC に基づく比較が意味をもつのは、同じ期間の同じ時系列に当てはめられた
モデル同士に限られる点に注意が必要です。さらに、状態空間モデルにおける
AIC
の差は、回帰効果と潜在成分の確率構造の変化の両方を反映します。したがって、AIC
は推論の唯一の根拠とするのではなく、パラメータ推定値とその不確実性と併せて
...
![](tempssm_manual_jp_files/figure-gfm/unnamed-chunk-17-1.png)<!-- -->
...
![](tempssm_manual_jp_files/figure-gfm/unnamed-chunk-18-2.png)<!-- -->
...
- 欠測した温度値には NA を使用し、対応する Year と Month
  の行は必ず保持してください。
