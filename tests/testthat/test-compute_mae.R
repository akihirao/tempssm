...
test_that("compute_mae returns NA for NULL inputs", {
  expect_true(is.na(tempssm:::.compute_mae(NULL, 1:3)))
  expect_true(is.na(tempssm:::.compute_mae(1:3, NULL)))
})


test_that("length mismatch triggers error", {
  expect_error(
    tempssm:::.compute_mae(1:3, 1:2),
...
test_that("empty vectors return NA", {
  mae <- tempssm:::.compute_mae(numeric(0), numeric(0))
